defmodule Colt.Services.Ingest.Dk.Cvr.GrowthRollup do
  @moduledoc """
  Recomputes `companies.{revenue_latest, employees_latest,
  revenue_growth_bucket}` from `annual_reports`.

  Identical SQL to `Colt.Services.Ingest.Ee.Rik.GrowthRollup` /
  `Colt.Services.Ingest.Fi.Prh.AnnualReports.recompute_growth/0` —
  source-agnostic so it picks up the new DK rows correctly. Kept in its
  own module so a crash here can be re-run with
  `Colt.Services.Ingest.Dk.Cvr.run(from: 3)` without re-fetching XBRLs.
  """
