defmodule Colt.Jobs.Enrichment.CheckWebsite do
  ...
  alias Colt.Services.Enrichment.{CheckAlive, Freshness, NoWebsite, Suppression, Transition}
  ...
  def perform(%Oban.Job{args: %{"campaign_company_id" => id}}) do
    with {:ok, cc} <- CampaignCompany.get(id),
      ...
      cond do
        ...
        # CSE call — reuse that verdict for this campaign.
        Freshness.website_search_fresh?(company) ->
          NoWebsite.run(cc, campaign, reason: "no website found (cached)")
